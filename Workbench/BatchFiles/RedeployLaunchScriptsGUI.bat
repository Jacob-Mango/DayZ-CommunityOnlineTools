...
for /f "delims=" %%a in ('call %batchFileDirectory%ExtractData.bat project.cfg ScriptsPrefix') do (
    set scriptsPrefix=%%a
)

for /f "delims=" %%a in ('call %batchFileDirectory%ExtractData.bat project.cfg GUIPrefix') do (
    set guiPrefix=%%a
)
