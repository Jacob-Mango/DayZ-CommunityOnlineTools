...
class JMExampleForm: JMFormBase
{
	...
	//! Live sort sweep (2026-09-24): T1-T9, M1-M4 and the removed Host/Round-2
	//! cards all came back blank, so this sweeps JMStatics.WINDOWS_CONTAINER's
	//! own sort by hand instead of fixed steps - see InitPreviewLab.
	protected UIActionSlider              m_LabSortSlider;
	protected UIActionText                m_LabSortStatus;

	//! Second, independent lever: the preview/map widgets' OWN sort, not their
	//! ancestor's - see BuildSweepPreview/BuildSweepMap.
	protected ItemPreviewWidget           m_LabSweepPreview;
	protected MapWidget                   m_LabSweepMap;
	protected UIActionSlider              m_LabWidgetSortSlider;
	protected UIActionText                m_LabWidgetSortStatus;

	//! Third, independent lever: two test map markers on the sweep map, same
	//! shape UIActionMap builds in production - real markers reportedly still
	//! do not show even with SORT_PREVIEW_OVERLAY applied, so this sweeps
	//! their sort by hand too instead of trusting the fixed constant.
	protected ref array<ref JMUIActionMapMarker> m_LabSweepMarkers;
	protected UIActionSlider              m_LabMarkerSortSlider;
	protected UIActionText                m_LabMarkerSortStatus;
	protected UIActionText                m_LabMarkerPosStatus;
	protected bool                        m_LabContinuousReposition;
	...
	protected void InitPreviewLab()
	{
		...
		// --- Live sort sweep (2026-09-24) ------------------------------------
		// Every fixed-step tentative above (T1-T9, M1-M4, and the removed
		// Host/Round-2 cards) came back blank, including T1 ("what COT
		// shipped") and H3 (inside a real UIScriptedMenu) - so this is no
		// longer about host or style, it is a live sweep of one number.
		// One preview and one map, built immediately (no button - both are on
		// screen the moment this window opens), and a slider + Apply button
		// that pushes an arbitrary sort straight onto JMStatics.WINDOWS_CONTAINER
		// so the threshold, if one exists, can be found by hand instead of
		// guessed at in fixed steps.
		UIActionCard sweepCard = UIActionManager.CreateCard( m_PanelPreviewLab, "Live sort sweep" );
		Widget sweepContent = sweepCard.GetContent();

		BuildSweepPreview( sweepContent );
		BuildSweepMap( sweepContent );

		//! Read the REAL current value, not JMUILayout.SORT_WINDOW - COTModule.c
		//! deliberately keeps WINDOWS_CONTAINER at its default sort (0), never
		//! SORT_WINDOW, because raising it blanks every RTT widget under it.
		//! A slider defaulted to SORT_WINDOW would misrepresent the starting
		//! state and silently apply that exact change the first time Apply is
		//! clicked - which is the last thing this card should do by surprise.
		int startingContainerSort = 0;
		if ( JMStatics.WINDOWS_CONTAINER )
			startingContainerSort = JMStatics.WINDOWS_CONTAINER.GetSort();

		m_LabSortSlider = UIActionManager.CreateSyncedSlider( sweepContent, "WINDOWS_CONTAINER sort", -2000, 40000, this, "OnChange_LabSortSlider" );
		m_LabSortSlider.SetCurrent( startingContainerSort );

		UIActionManager.CreateButton( sweepContent, "Apply container sort", this, "Click_LabApplySort" );

		m_LabSortStatus = UIActionManager.CreateText( sweepContent, "WINDOWS_CONTAINER sort is currently:", "?" );
		RefreshLabSortStatus();

		// A second, separate lever: the preview/map WIDGETS' own local sort
		// (what JMUILayout.SORT_PREVIEW used to drive before it was disabled),
		// not their ancestor's. Two independent numbers, two independent
		// sliders - do not conflate them. Read the REAL applied value (now
		// SORT_PREVIEW, since BuildSweepPreview/BuildSweepMap above apply it
		// the same way production does), not a hardcoded guess - the
		// WINDOWS_CONTAINER slider's hardcoded default was exactly this same
		// bug.
		int startingWidgetSort = 0;
		if ( m_LabSweepPreview )
			startingWidgetSort = m_LabSweepPreview.GetSort();

		m_LabWidgetSortSlider = UIActionManager.CreateSyncedSlider( sweepContent, "preview/map widget sort", -2000, 40000, this, "OnChange_LabWidgetSortSlider" );
		m_LabWidgetSortSlider.SetCurrent( startingWidgetSort );

		UIActionManager.CreateButton( sweepContent, "Apply widget sort", this, "Click_LabApplyWidgetSort" );

		m_LabWidgetSortStatus = UIActionManager.CreateText( sweepContent, "preview/map widget sort is currently:", "?" );
		RefreshLabWidgetSortStatus();

		// A third, independent lever: the two test MARKERS' own sort, not the
		// map's - real markers still reportedly do not show even at
		// SORT_PREVIEW_OVERLAY, so sweep this by hand too.
		m_LabMarkerSortSlider = UIActionManager.CreateSyncedSlider( sweepContent, "marker sort", -2000, 40000, this, "OnChange_LabMarkerSortSlider" );
		m_LabMarkerSortSlider.SetCurrent( JMUILayout.SORT_PREVIEW_OVERLAY );

		UIActionManager.CreateButton( sweepContent, "Apply marker sort", this, "Click_LabApplyMarkerSort" );

		m_LabMarkerSortStatus = UIActionManager.CreateText( sweepContent, "marker sort is currently:", "?" );
		RefreshLabMarkerSortStatus();
	}

	//! Sweep target: item preview. Same shape BuildPreviewTentative uses, but
	//! keeps the widget reference so the widget-sort slider below has
	//! something to apply to.
	protected void BuildSweepPreview( Widget parent )
	{
		UIActionManager.CreateText( parent, "Sweep target: item preview", "" );

		Widget row = UIActionManager.CreateRow( parent, 72 );
		Widget host = g_Game.GetWorkspace().CreateWidgets( "JM/COT/GUI/layouts/previewlab/T2_preview_panel_blank.layout", row );

		if ( !host )
			return;

		if ( !Class.CastTo( m_LabSweepPreview, host.FindAnyWidget( "lab_preview" ) ) )
			return;
		...
		m_LabSweepPreview.SetItem( ent );
		m_LabSweepPreview.SetView( ent.GetViewIndex() );
		m_LabSweepPreview.SetModelPosition( Vector( 0, 0, 1 ) );
		m_LabSweepPreview.SetModelOrientation( vector.Zero );
		m_LabSweepPreview.Show( true );

		//! Match production exactly: JMObjectSpawnerForm.OnCreate calls this
		//! same SetSort immediately after creating its ItemPreviewWidget -
		//! the layout's own "priority 151" (object_preview and lab_preview
		//! declare the identical value) never survives past this call in
		//! production, so the lab should not leave it sitting at 151 either.
		m_LabSweepPreview.SetSort( JMUILayout.SORT_PREVIEW );
	}

	//! Sweep target: map. Same shape BuildMapTentative uses, but keeps the
	//! widget reference for the same reason as BuildSweepPreview above.
	protected void BuildSweepMap( Widget parent )
	{
		UIActionManager.CreateText( parent, "Sweep target: map", "" );

		Widget row = UIActionManager.CreateRow( parent, 208 );
		Widget host = g_Game.GetWorkspace().CreateWidgets( "JM/COT/GUI/layouts/previewlab/M1_map_bare.layout", row );

		if ( !host )
			return;

		if ( !Class.CastTo( m_LabSweepMap, host.FindAnyWidget( "lab_map" ) ) )
			return;

		LabCenterMap( m_LabSweepMap );

		//! Match production exactly: UIActionMap.OnInit calls this same
		//! SetSort right after finding action_map - the map here should
		//! start at that same applied value, not the layout's raw default.
		m_LabSweepMap.SetSort( JMUILayout.SORT_PREVIEW );

		//! Three test markers, same shape UIActionMap/JMUIActionMapMarker builds
		//! in production. A and B are children of `host` - the same panel the
		//! MapWidget itself sits in, exactly how production parents real
		//! markers (see UIActionMap.CreateOrReuseMarkerWidget). C is parented
		//! to `row` instead - `host`'s OWN parent, i.e. a SIBLING of the host
		//! panel rather than a child of it. That is exactly how production's
		//! own action_map_ring (the border overlay that DOES draw on top of
		//! the map, no special sort) relates to action_map_host - see
		//! UIActionMap.layout. If C draws over the map and A/B do not
		//! regardless of sort, the fix is re-parenting markers one level up,
		//! not a sort value.
		m_LabSweepMarkers = new array<ref JMUIActionMapMarker>;

		vector center = m_LabSweepMap.GetMapPos();
		string pinIcon = JMConstants.Lucide( "map-pin" );

		JMUIActionMapMarker markerA = new JMUIActionMapMarker( host, null, "sweep_a", center, "A: child of host", JMTheme.ACCENT, pinIcon );
		JMUIActionMapMarker markerB = new JMUIActionMapMarker( host, null, "sweep_b", center + Vector( 80, 0, 80 ), "B: child of host", JMTheme.ACCENT, pinIcon );
		JMUIActionMapMarker markerC = new JMUIActionMapMarker( row, null, "sweep_c", center + Vector( -80, 0, -80 ), "C: sibling of host", JMTheme.DANGER, pinIcon );

		markerA.UpdateScreenPosition( m_LabSweepMap );
		markerB.UpdateScreenPosition( m_LabSweepMap );
		markerC.UpdateScreenPosition( m_LabSweepMap );

		m_LabSweepMarkers.Insert( markerA );
		m_LabSweepMarkers.Insert( markerB );
		m_LabSweepMarkers.Insert( markerC );

		//! D/E: the same sibling-vs-child question as C, but with every other
		//! variable removed - no JMUIActionMapMarker, no icon file, no
		//! MapToScreen math, no marker-sort slider (sort pinned to 9999, the
		//! ordinary "beat everything in this parent" convention, not the
		//! narrow SORT_PREVIEW/SORT_PREVIEW_OVERLAY band). Just a plain bright
		//! square at a fixed pixel offset. If NEITHER D nor E ever shows, the
		//! map is not just out-drawing markers - it is out-drawing anything,
		//! which rules out marker-specific causes (icon, positioning) entirely.
		Widget markerD = UIActionManager.CreatePanel( host, JMTheme.SUCCESS_DIM );
		markerD.SetFlags( WidgetFlags.HEXACTPOS | WidgetFlags.VEXACTPOS | WidgetFlags.HEXACTSIZE | WidgetFlags.VEXACTSIZE, true );
		markerD.SetPos( 20, 20 );
		markerD.SetSize( 40, 40 );
		markerD.SetSort( 9999 );

		Widget markerE = UIActionManager.CreatePanel( row, JMTheme.SUCCESS_DIM );
		markerE.SetFlags( WidgetFlags.HEXACTPOS | WidgetFlags.VEXACTPOS | WidgetFlags.HEXACTSIZE | WidgetFlags.VEXACTSIZE, true );
		markerE.SetPos( 70, 20 );
		markerE.SetSize( 40, 40 );
		markerE.SetSort( 9999 );

		//! A one-off manual reposition (deferred a frame, or called from a
		//! button) turned out to HIDE otherwise-visible markers - the default,
		//! immediate UpdateScreenPosition call above is what actually works.
		//! That is a real clue, not a dead end: production's UIActionMap
		//! does not position a marker once and leave it - TickMarkers() calls
		//! UpdateScreenPosition again EVERY FRAME the view has moved (see
		//! UIActionMap.c), which is exactly the kind of repeated call that
		//! just broke things here. So this toggles CONTINUOUS repositioning
		//! on A/B/C, the same way a real window's Update() loop drives a real
		//! UIActionMap, instead of a single deferred call - if they vanish
		//! once ticking starts and stay gone, the bug is in
		//! UpdateScreenPosition/TickMarkers itself, not in first-frame timing.
		UIActionManager.CreateButton( row, "Toggle continuous reposition (mimics production TickMarkers)", this, "Click_LabToggleContinuousReposition" );
		m_LabMarkerPosStatus = UIActionManager.CreateText( row, "continuous reposition:", "off" );
	}

	void Click_LabToggleContinuousReposition( UIEvent eid, UIActionBase action )
	{
		if ( eid != UIEvent.CLICK )
			return;

		m_LabContinuousReposition = !m_LabContinuousReposition;

		if ( m_LabMarkerPosStatus )
			m_LabMarkerPosStatus.SetText( m_LabContinuousReposition.ToString() );
	}

	//! Mirrors what JMTeleportForm.Update() -> m_Map.TickMarkers() does every
	//! frame for a real UIActionMap - calls UpdateScreenPosition again on
	//! every sweep marker, the same call the "default, working" positioning
	//! above also uses, just repeated instead of one-shot. JMFormBase's own
	//! per-frame hook is Update(), not OnUpdate() - that is JMFormTab's.
	override void Update()
	{
		super.Update();

		if ( !m_LabContinuousReposition || !m_LabSweepMap || !m_LabSweepMarkers )
			return;

		foreach ( JMUIActionMapMarker marker : m_LabSweepMarkers )
		{
			if ( marker )
				marker.UpdateScreenPosition( m_LabSweepMap );
		}
	}

	//! Both sliders only store the pending value; nothing is applied until its
	//! own Apply button is clicked, so dragging past the value that blanks the
	//! widgets does not lose the last one that worked.
	void OnChange_LabSortSlider( UIEvent eid, UIActionBase action )
	{
	}

	void OnChange_LabWidgetSortSlider( UIEvent eid, UIActionBase action )
	{
	}

	void Click_LabApplySort( UIEvent eid, UIActionBase action )
	{
		if ( eid != UIEvent.CLICK || !m_LabSortSlider )
			return;

		if ( !JMStatics.WINDOWS_CONTAINER )
			return;

		int sort = (int)Math.Round( m_LabSortSlider.GetCurrent() );

		JMStatics.WINDOWS_CONTAINER.SetSort( sort );
		Print( "[COT-LAB] live sweep: WINDOWS_CONTAINER sort set to " + sort );

		RefreshLabSortStatus();
	}

	//! Applies to the preview/map widgets THEMSELVES, not their ancestor -
	//! the lever JMUILayout.SORT_PREVIEW used to be wired to before it was
	//! disabled (see UIActionMap.c / JMObjectSpawnerForm.c's COT_LAB_SORT_TESTING
	//! notes).
	void Click_LabApplyWidgetSort( UIEvent eid, UIActionBase action )
	{
		if ( eid != UIEvent.CLICK || !m_LabWidgetSortSlider )
			return;

		int sort = (int)Math.Round( m_LabWidgetSortSlider.GetCurrent() );

		if ( m_LabSweepPreview )
			m_LabSweepPreview.SetSort( sort );

		if ( m_LabSweepMap )
			m_LabSweepMap.SetSort( sort );

		Print( "[COT-LAB] live sweep: preview/map widget sort set to " + sort );

		RefreshLabWidgetSortStatus();
	}

	protected void RefreshLabWidgetSortStatus()
	{
		if ( !m_LabWidgetSortStatus )
			return;

		int sort = -1;
		if ( m_LabSweepPreview )
			sort = m_LabSweepPreview.GetSort();

		m_LabWidgetSortStatus.SetText( sort.ToString() );
	}

	void OnChange_LabMarkerSortSlider( UIEvent eid, UIActionBase action )
	{
	}

	void Click_LabApplyMarkerSort( UIEvent eid, UIActionBase action )
	{
		if ( eid != UIEvent.CLICK || !m_LabMarkerSortSlider || !m_LabSweepMarkers )
			return;

		int sort = (int)Math.Round( m_LabMarkerSortSlider.GetCurrent() );
		...
		Print( "[COT-LAB] live sweep: marker sort set to " + sort );

		RefreshLabMarkerSortStatus();
	}

	protected void RefreshLabMarkerSortStatus()
	{
		if ( !m_LabMarkerSortStatus )
			return;

		int sort = -1;
		if ( m_LabSweepMarkers && m_LabSweepMarkers.Count() > 0 && m_LabSweepMarkers[0] )
			sort = m_LabSweepMarkers[0].GetSort();

		m_LabMarkerSortStatus.SetText( sort.ToString() );
	}

	protected void RefreshLabSortStatus()
	{
		if ( !m_LabSortStatus )
			return;
		...
		int sort = -1;
		if ( JMStatics.WINDOWS_CONTAINER )
			sort = JMStatics.WINDOWS_CONTAINER.GetSort();

		m_LabSortStatus.SetText( sort.ToString() );
	}
