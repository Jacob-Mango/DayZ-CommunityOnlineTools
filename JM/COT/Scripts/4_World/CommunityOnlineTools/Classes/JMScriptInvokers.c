class JMScriptInvokers
{
	static ref ScriptInvoker COT_ON_OPEN = new ScriptInvoker;
	static ref ScriptInvoker COT_ON_CLOSE = new ScriptInvoker;
	static ref ScriptInvoker COT_ON_SELECTED_PLAYER_CHANGE = new ScriptInvoker;
	static ref ScriptInvoker COT_ON_PERMISSIONS_UPDATED = new ScriptInvoker;
	static ref ScriptInvoker COT_ON_THEME_CHANGED = new ScriptInvoker;
	static ref ScriptInvoker ON_PLAYER_CONTEXT_MENU = new ScriptInvoker;
	static ref ScriptInvoker MENU_PLAYER_BUTTON = new ScriptInvoker;
	static ref ScriptInvoker MENU_PLAYER_CHECKBOX = new ScriptInvoker;
	static ref ScriptInvoker ADD_OBJECT = new ScriptInvoker;
	static ref ScriptInvoker REMOVE_OBJECT = new ScriptInvoker;
	static ref ScriptInvoker ON_DELETE_ALL = new ScriptInvoker;
	static ref ScriptInvoker ESP_VIEWTYPE_CHANGED = new ScriptInvoker;
}

