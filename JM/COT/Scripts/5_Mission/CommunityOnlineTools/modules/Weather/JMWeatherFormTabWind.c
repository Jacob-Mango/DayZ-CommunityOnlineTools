...
class JMWeatherFormTabWind
{
	...
	{
		m_ScrollerWind = UIActionManager.CreateScroller( parent );
		...
		m_SliderWindMagnitude = UIActionManager.CreateSlider( windBody, "#STR_COT_WEATHER_MAGNITUDE", 0, JMWeatherForm.WIND_SCALE_MAX, this, "OnChange_Wind" );
	...
	void SetWindValues( JMWeatherPreset preset, bool actual )
	{
		...
		{
			m_SliderWindDirection.SetCurrent( JMWeatherForm.PI2DEG( directionRad ) );
	...
	void ReadInto( JMWeatherPreset preset, float transition, float duration )
	{
		...
		if ( m_SliderWindDirection )
			preset.PWindDirection.Forecast = JMWeatherForm.DEG2PI( m_SliderWindDirection.GetCurrent() );
	...
	void UpdateWindDirectionSlider()
	{
		...
		m_SliderWindDirection.SetText( JMWeatherForm.CardinalFor( direction ) + "  " + degrees );
	}
	...
	void ApplyWind()
	{
		...
		if ( m_SliderWindDirection )
			m_Form.m_Module.SetWindDirection( JMWeatherForm.DEG2PI( m_SliderWindDirection.GetCurrent() ), windTransition, windDuration );
