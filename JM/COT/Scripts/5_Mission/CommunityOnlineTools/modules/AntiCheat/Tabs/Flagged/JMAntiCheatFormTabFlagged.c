...
class JMAntiCheatFormTabFlagged: JMFormTab
{
	...
	override void OnCreate( Widget panel )
	{
		...
		UIActionCard card = UIActionManager.CreateScrollCard( panel, "Cheat Detections & Flagged Players", m_FlaggedScroller );
	...
	void OnClick_ToggleSubGroup( UIActionBase action )
	{
	...
	protected void ApplyChevron( UIActionBase button, bool expanded )
	{
		...
		if ( expanded )
		{
			...
			icon.SetTooltip( "#STR_COT_ANTICHEAT_HIDE" );

			return;
		}

		icon.SetImage( JMConstants.Lucide( "chevron-down" ) );
		icon.SetTooltip( "#STR_COT_ANTICHEAT_SHOW" );
	}
	...
	void OnClick_Clear( UIActionBase action )
	{
		...
		m_Form.ConfirmAction( "#STR_COT_ANTICHEAT_CLEAR_ANTI_CHEAT_FLAGS", "Clear every anti-cheat flag for " + PendingPlayerName() + "?", "ConfirmClear" );
	}
	...
	void OnClick_Freeze( UIActionBase action )
	{
		JMAntiCheatClearData data;
		if ( !Class.CastTo( data, action.GetData() ) )
			return;
	...
	void OnClick_Kick( UIActionBase action )
	{
		JMAntiCheatClearData data;
		if ( !Class.CastTo( data, action.GetData() ) )
			return;
		...
		m_Form.ConfirmAction( "#STR_COT_ANTICHEAT_KICK_PLAYER", "Kick " + PendingPlayerName() + " from the server?", "ConfirmKick" );
	}
	...
	void OnClick_Ban( UIEvent eid, UIActionBase action )
	{
		...
		m_Form.ConfirmAction( "#STR_COT_ANTICHEAT_BAN_PLAYER", "Ban " + PendingPlayerName() + " from the server?", "ConfirmBan" );
	}

	void OnClick_PlayerPage( UIActionBase action )
	{
		JMAntiCheatClearData data;
		if ( !Class.CastTo( data, action.GetData() ) )
			return;
	...
	void Rebuild( array< string > order, map< string, ref JMAntiCheatFlag > flags, string filter, bool lowestFirst )
	{
		...
		UIActionManager.ClearChildren( m_FlaggedContent );
		Widget body = m_FlaggedContent;

		if ( !order || order.Count() == 0 )
		{
			UIActionManager.CreateEmptyState( body, "#STR_COT_ANTICHEAT_NO_FLAGS_DETECTED", "#STR_COT_ANTICHEAT_SERVER_IS_CLEAN_DETECTIONS_APPEAR_HERE" );
			m_FlaggedScroller.UpdateScroller();
			return;
		}
		...
		foreach ( string guid : sortedOrder )
		{
			...
			if ( !matcher.Matches( flag.PlayerName ) && !matcher.Matches( guid ) )
				continue;
			...
			Widget rowIdentity = UIActionManager.CreateRow( body, ROW_IDENTITY_PX );
			...
			scoreBadge.SetTooltip( "#STR_COT_ANTICHEAT_CURRENT_CHEAT_DETECTION_SCORE_IT_DECAYS" );
			...
			UIActionImageButton freezeBtn = UIActionManager.CreateIconButton( actionWrap, JMConstants.Lucide( "snowflake" ), this, "" );
			if ( freezeBtn ) freezeBtn.SetOnClick( this, "OnClick_Freeze" );
			...
			freezeBtn.SetTooltip( "#STR_COT_ANTICHEAT_FREEZE_PLAYER_MOVEMENT" );

			UIActionImageButton kickBtn = UIActionManager.CreateIconButton( actionWrap, JMConstants.Lucide( "user-x" ), this, "" );
			if ( kickBtn ) kickBtn.SetOnClick( this, "OnClick_Kick" );
			kickBtn.SetFixedSize( JMFormBase.ICON_BUTTON_PX, JMFormBase.ICON_BUTTON_PX );
			kickBtn.SetData( new JMAntiCheatClearData( guid ) );
			kickBtn.SetTooltip( "#STR_COT_ANTICHEAT_KICK_PLAYER_FROM_SERVER" );
			...
			banBtn.SetTooltip( "#STR_COT_ANTICHEAT_BAN_PLAYER_FROM_SERVER" );

			UIActionImageButton pageBtn = UIActionManager.CreateIconButton( actionWrap, JMConstants.Lucide( "user" ), this, "" );
			if ( pageBtn ) pageBtn.SetOnClick( this, "OnClick_PlayerPage" );
			pageBtn.SetFixedSize( JMFormBase.ICON_BUTTON_PX, JMFormBase.ICON_BUTTON_PX );
			pageBtn.SetData( new JMAntiCheatClearData( guid ) );
			pageBtn.SetTooltip( "#STR_COT_ANTICHEAT_OPEN_PLAYER_DETAIL_PAGE" );

			UIActionImageButton clearBtn = UIActionManager.CreateIconButton( actionWrap, JMConstants.ICON_TRASH_CAN, this, "" );
			if ( clearBtn ) clearBtn.SetOnClick( this, "OnClick_Clear" );
			clearBtn.SetFixedSize( JMFormBase.ICON_BUTTON_PX, JMFormBase.ICON_BUTTON_PX );
			clearBtn.SetData( new JMAntiCheatClearData( guid ) );
			clearBtn.SetTooltip( "#STR_COT_ANTICHEAT_CLEAR_ANTI_CHEAT_FLAGS_FOR_THIS" );
			m_Form.BindPermission( clearBtn, JMConstants.PERM_ANTICHEAT_CLEAR );

			// -- Line 2: guid ---------------------------------------------------
			Widget rowGuid = UIActionManager.CreateRow( body, ROW_GUID_PX );
			...
			if ( hasEvents )
			{
				...
				foreach ( JMAntiCheatGroupEntry group : detectionGroups )
				{
					...
					if ( group.TotalPoints > 0 )
					{
						...
						evtDelta.SetTooltip( "#STR_COT_ANTICHEAT_SCORE_THESE_DETECTIONS_ADDED_WHEN_THEY" );
					}
					...
					UIActionImageButton subExpandBtn = UIActionManager.CreateIconButton( subExpandWrap, subIcon, this, "" );
					if ( subExpandBtn ) subExpandBtn.SetOnClick( this, "OnClick_ToggleSubGroup" );
					...
					{
						Widget occRow = UIActionManager.CreateRow( occurrenceBody, OCCURRENCE_ROW_PX );
		...
		{
			UIActionManager.CreateEmptyState( body, "No flagged players match filter '" + filter + "'.", "", 40 );
		}

		m_FlaggedScroller.UpdateScroller();
	}

	protected int ScoreToColor( int score )
	{
