...
class JMLootAnalysisFormTabItemScan: JMFormTab
{
	...
	override void OnCreate(Widget panel)
	{
		...
		m_ScanMapWrapper   = UIActionManager.CreatePanel(panel, 0x00000000);
		m_ScanStatsWrapper = UIActionManager.CreatePanel(panel, 0x00000000, JMLootAnalysisForm.STATS_H);

		UIActionCard mapCard = UIActionManager.CreateCard(m_ScanMapWrapper, "#STR_COT_LOOTANALYSIS_ITEM_SCAN_MAP_TITLE");

		m_ShowOnMapButton = mapCard.AddCardHeaderAction(JMConstants.Lucide("search"), this, "OnClick_ShowOnMap", "Show every world instance of the selected item");
	...
	//! actually arrives. Re-checked every OnUpdate() while the tab is focused; the
	...
	void OnClick_DeleteAll(UIEvent eid, UIActionBase action)
	{
		...
		m_Form.ConfirmAction( "#STR_COT_LOOTANALYSIS_DELETE_ALL_ITEMS", "Are you sure you want to delete ALL instances of: " + className + "?", "DeleteAll_Yes" );
	}
	...
	void RefreshStats()
	{
		...
		m_ScanStatsList.SetValue("Items", count.ToString());
	...
	void OnClick_CopyScanStats(UIEvent eid, UIActionBase action)
	{
		...
		if (m_ScanStatsList)
			COTFeedback.Copy(m_Form.StatsListToText(m_ScanStatsList));
	}
	...
	//! Leaving the tab must not leave the floating preview hanging over the other one.
