...
class JMLootAnalysisFormTabDistribution: JMFormTab
{
	...
	override void OnCreate(Widget panel)
	{
		...
		m_DistMapWrapper    = UIActionManager.CreatePanel(panel, 0x00000000);
		m_DistStatsWrapper  = UIActionManager.CreatePanel(panel, 0x00000000, JMLootAnalysisForm.STATS_H);
		...
		m_DistStatsCard = UIActionManager.CreateCard(m_DistStatsWrapper, "#STR_COT_LOOTANALYSIS_ITEM_INFO");
	...
	//! See JMLootAnalysisFormTabItemScan.OnUpdate() - same measure-and-relayout
	...
	void OnClick_CopyDistStats(UIEvent eid, UIActionBase action)
	{
		...
		if (m_DistStatsList)
			COTFeedback.Copy(m_Form.StatsListToText(m_DistStatsList));
	}
}
