...
class JMRoleManagerFormTabPlayers
{
	...
	void RebuildPlayerEditor()
	{
		...
		Widget rowGuid = UIActionManager.CreatePanel( idBody, 0x00000000, JMFormBase.IDENTITY_ROW_HEIGHT );
		...
		Widget rowSteam = UIActionManager.CreatePanel( idBody, 0x00000000, JMFormBase.IDENTITY_ROW_HEIGHT );
	...
	void OnClick_EditRoleNameRestriction( UIEvent eid, UIActionBase action )
	{
		...
		m_Form.CreateConfirmation_Two( JMConfirmationType.EDIT, restrictionTitle, restrictionBody, "#STR_COT_GENERIC_CANCEL", "", "#STR_COT_GENERIC_CONFIRM", "ConfirmRoleNameRestriction" );
	}
