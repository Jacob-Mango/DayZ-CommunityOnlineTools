...
class JMESPFilterGroup: UIActionData
{
	...
	//! Kept so a group that no view type landed in can be hidden outright
	//! once the rows are handed out - the groups are built before that is
	//! known, and an empty one otherwise leaves a header and a hairline
	//! with nothing between them.
	Widget GroupHeaderRow;
	Widget GroupDivider;
...
//!
//! Each JMESPViewTypeWidget row holds this class as its owner and calls
//! OnRightClick_CategoryRow / OnChange_CategoryRow / OpenCategoryColourPopup on it.
class JMESPFormTabFilters: JMFormTab
{
	...
	protected void SetGroupChecked( JMESPFilterGroup group, bool state )
	{
		if ( !group )
			return;
	...
	protected void SetGroupExpanded( JMESPFilterGroup group, bool expanded, bool animate = true )
	{
	...
	override void OnCreate( Widget panel )
	{
		...
		m_ESPListScroller = UIActionManager.CreateScroller( panel );
		...
		m_EspSelectAll = UIActionManager.CreateFeedbackButton( rowSelectors, "#STR_COT_ESP_MODULE_ACTION_SELECT_ALL", "#STR_COT_SELECTED", JMConstants.ICON_CHECK_MARK, this, "" );
		if ( m_EspSelectAll ) m_EspSelectAll.SetOnClick( this, "OnClick_ESPSelectAll" );
		m_EspSelectAll.SetIcon( JMConstants.ICON_SELECT_ALL );
		m_EspSelectAll.SetTooltip( "#STR_COT_ESP_MODULE_TT_ESP_SELECT_ALL" );
		m_EspDeselectAll = UIActionManager.CreateFeedbackButton( rowSelectors, "#STR_COT_ESP_MODULE_ACTION_DESELECT_ALL", "#STR_COT_CLEARED", JMConstants.ICON_CHECK_MARK, this, "" );
		if ( m_EspDeselectAll ) m_EspDeselectAll.SetOnClick( this, "OnClick_ESPDeselectAll" );
		...
		m_Form.AddOverlay( m_CategoryPicker );

		UIActionManager.CreateDivider( container, JMTheme.DIVIDER_MEDIUM, 1 );
	...
	//! Group labels in GROUP_* order. A method rather than a field so the
	//! objects tab's category filter names its choices from the same list
	//! these headers use, instead of keeping a second copy in step by hand.
	static TStringArray GroupLabels()
	{
		TStringArray labels = {
	...
	//! Group icons in GROUP_* order, shared with the objects tab's category
	//! filter for the same reason GroupLabels is.
	//!
	//! One line, not a block: Enforce cannot span an initializer that holds
	//! calls across lines.
	static TStringArray GroupIcons()
	{
		TStringArray icons = { JMConstants.Lucide( "users" ), JMConstants.Lucide( "car" ), JMConstants.Lucide( "swords" ), JMConstants.Lucide( "package" ), JMConstants.Lucide( "mountain" ), JMConstants.Lucide( "shapes" ) };

		return icons;
	}

	protected void BuildFilterGroups( Widget container )
	{
		TStringArray groupLabels = GroupLabels();
		TStringArray groupIcons = GroupIcons();
		...
		for ( int g = 0; g < GROUP_COUNT; g++ )
		{
			...
			group.GroupHeaderRow = headerRow;
			...
			group.GroupDivider = UIActionManager.CreateDivider( container, JMTheme.DIVIDER_MEDIUM, 1 );
		...
		//! All shut by default: six open groups is a page of scrolling before
		//! the first row of anything, and every group is one click away.
		for ( int o = 0; o < m_FilterGroups.Count(); o++ )
			SetGroupExpanded( m_FilterGroups[o], false, false );

		RefreshGroupHeaders();
	}

	//! A group no view type landed in has nothing to fold open, so its
	//! header and hairline are all it would contribute - which is what the
	//! stray line under the last group was. The last group still standing
	//! drops its hairline too: a divider is there to separate one block
	//! from the next, and past the final block there is no next.
	protected void HideEmptyGroups()
	{
		int lastVisible = -1;

		for ( int i = 0; i < m_FilterGroups.Count(); i++ )
		{
			JMESPFilterGroup group = m_FilterGroups[i];

			if ( group.GroupRows.Count() > 0 )
			{
				lastVisible = i;
				continue;
			}

			if ( group.GroupHeaderRow )
				group.GroupHeaderRow.Show( false );

			if ( group.GroupFold )
				group.GroupFold.Hide();

			if ( group.GroupDivider )
				group.GroupDivider.Show( false );
		}

		if ( lastVisible >= 0 && m_FilterGroups[lastVisible].GroupDivider )
			m_FilterGroups[lastVisible].GroupDivider.Show( false );
	}
	...
	static int GroupIndexFor( JMESPViewType viewType )
	{
	...
	#ifndef DAYZ_1_29
		if ( t.IsInherited( JMESPViewTypeMotorbike ) )
	...
	protected void ApplyCategoryFilter( string text )
	{
		string needle = text;
		needle.ToLower();
	...
	void OnClick_CategoryMenu( UIEvent eid, UIActionBase action )
	{
		...
		if ( id == CAT_MENU_COPYPERM )
			COTFeedback.Copy( "ESP.View." + m_CategoryMenuTarget.GetViewType().Permission );

		RefreshGroupHeaders();
	}

	protected JMESPFilterGroup GroupOf( JMESPViewTypeWidget row )
	{
	...
	protected void EnableOnly( JMESPViewTypeWidget target )
	{
		foreach ( JMESPViewTypeWidget row : m_ESPTypeList )
		{
			if ( row.IsPermitted() )
	...
	protected void ResetOneColour( JMESPViewTypeWidget row )
	{
	...
	void OnClick_ESPDeselectAll( UIActionBase action )
	{
	...
	void OnClick_ESPSelectAll( UIActionBase action )
	{
		foreach ( JMESPViewTypeWidget espType: m_ESPTypeList )
		{
			if ( !espType.IsFiltered() && espType.IsPermitted() )
