...
class JMWebhookSection : Managed
{
	...
	protected void Build( JMWebhookConnectionGroup group, array< string > allTypes )
	{
		...
		saveBtn.SetTooltip( "#STR_COT_WEBHOOK_SAVE_CHANGES_TO_THIS_WEBHOOK" );

		UIActionManager.CreateDivider( m_RootSpacer, 0xFF3A3A3A, 1 );
		...
		m_Form.UpdatePermission( m_FilterGUIDEdit, JMConstants.PERM_WEBHOOK_MANAGE_URL_EDIT );
		m_Form.UpdatePermission( m_FilterRoleEdit, JMConstants.PERM_WEBHOOK_MANAGE_URL_EDIT );
	...
	protected void RebuildTypes( JMWebhookConnectionGroup group, array< string > allTypes )
	{
		for ( int i = 0; i < group.Count(); i++ )
		{
			...
			m_Form.UpdatePermission( cb,            JMConstants.PERM_WEBHOOK_MANAGE_TYPE_STATE );
