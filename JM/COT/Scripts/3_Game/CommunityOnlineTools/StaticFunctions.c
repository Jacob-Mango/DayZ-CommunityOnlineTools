...
static TVectorArray GetSpawnPoints()
{
	...
	{
		vector tempPosition = g_Game.ConfigGetVector(string.Format("CfgWorlds %1 centerPosition", worldName));
		spawnPoints.Insert(Vector(tempPosition[0], tempPosition[2], tempPosition[1]));
	}
...
static vector SnapToGround(vector pos, bool aboveWater = true)
{
	float pos_x = pos[0];
	float pos_z = pos[2];
	float pos_y;
