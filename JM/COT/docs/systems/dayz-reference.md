...
| Layer | Path | Role |
|-------|------|------|
| 1_core | scripts/1_core/ | Engine constants, proto wrappers (C++ bindings) (45 files) |
| 2_gamelib | scripts/2_gamelib/ | Abstract game framework (Game, InputManager) (13 files) |
| 3_game | scripts/3_game/ | Core game systems (440 files) |
| 4_world | scripts/4_world/ | Game content & mechanics (2,317 files) |
| 5_mission | scripts/5_mission/ | Mission/UI (219 files) |

(File counts as of 2026-09-21; the scripts folder tracks the installed game build, so they drift with each update.)
...
│   └── PlayerBase     → scripts/4_world/entities/manbase/playerbase.c  (~10,100 lines)
...
```c
g_Game            // DayZGame singleton - use this. GetGame() still exists (3_game/gameplay.c) but is the older spelling
g_Game.GetPlayer()   // local player (returns Man; cast to PlayerBase)
g_Game.GetWeather()  // Weather singleton (proto native)
GetPlayer()       // COT's global helper (4_world/.../StaticFunctions.c): local PlayerBase, not vanilla
```
...
// Typed stat access (PlayerStat<T> getters that exist on PlayerBase):
GetStatEnergy()      // PlayerStat<float>  0–MAX
GetStatWater()       // PlayerStat<float>  0–MAX
GetStatStamina()     // PlayerStat<float>
GetStatHeatBuffer()  // PlayerStat<float>  -30..30
GetStatHeatComfort() // PlayerStat<float>
GetStatTremor()      // PlayerStat<float>
GetStatToxicity() / GetStatDiet() / GetStatSpecialty()   // PlayerStat<float>
GetStatWet()         // PlayerStat<int>
GetStatBloodType()   // PlayerStat<int>

// Health, blood and shock are NOT PlayerStats - they are entity damage zones:
float hp    = player.GetHealth( "GlobalHealth", "Health" );   // 0–100
float blood = player.GetHealth( "GlobalHealth", "Blood" );    // 0–5000
float shock = player.GetHealth( "GlobalHealth", "Shock" );    // 0–100
player.SetHealth( "GlobalHealth", "Health", 100.0 );
...
// Set weather:
g_Game.GetWeather().GetOvercast().Set( 0.0, 120.0, 60.0 );  // clear sky
