...
// Reading
string value = txt.GetText();      // or action.GetText() in handler (UIActionBase)

// Setting
txt.SetText( "default text" );     // or SetText( float ) for numeric boxes
...
The authoritative list is `Scripts/Data/Inputs.xml` (45 actions today). Defaults, by group:
...
| `UACOTToggleMapEditor` | RAlt+B | Map Editor module |
| `UACOTToggleEntity` | RAlt+P | Object Spawner module |
| `UACOTToggleSelected` | RAlt+M | Declared, but no module currently uses it as its `InputToggle` |
...
| `UACOTToggleServerInfo` | RAlt+N | Declared, but the Server Stats module sets no `InputToggle` |
| `UACOTToggleCompensations` | RAlt+, | Compensations module |
| `UACOTToggleWebhook` | RAlt+. | Webhook module |
| `UACameraToolToggleCamera` | Insert | Enter/exit freecam |
| `UACameraToolZoomForwards` / `UACameraToolZoomBackwards` | Ctrl+ScrollUp / Ctrl+ScrollDown | Camera zoom |
| `UACameraToolSpeedIncrease` / `UACameraToolSpeedDecrease` | ScrollUp / ScrollDown | Camera speed |
| `UATeleportModuleTeleportCursor` | H | Cursor teleport |
| `UATeleportModuleWorldToModelLookAt` | L | Teleport to the model the camera looks at |
| `UAPlayerModuleGodMode` | U | Toggle godmode |
| `UAPlayerModuleStopSpectating` | Numpad1 | Exit spectate |
| `UAPlayerModuleFreezePlayer`, `...Heal`, `...Invisibility`, `...CanBeTargetedByAI`, `...UnlimitedStamina`, `...UnlimitedAmmo`, `...AdminNV` | (unbound) | Player-module toggles the user can bind |
| `UAObjectModuleDeleteOnCursor` | Delete | Delete at cursor |
| `UAObjectModuleSpawnInfected` / `...SpawnAnimal` / `...SpawnWolf` | (unbound) | Spawn a creature at the cursor |
| `UAMapEditorMoveForward/Backward/Left/Right/Up/Down` | W / S / A / D / E / Q | Map Editor freecam movement |
| `UAMapEditorBoost` / `UAMapEditorSlow` / `UAMapEditorLook` | LShift / LCtrl / RMB | Map Editor freecam speed and mouse-look |

Key name constants: `kEnd`, `kInsert`, `kDelete`, `kF1`-`kF12`, `kT`-`kZ`, `kLControl` / `kLCtrl`, `kRMenu` (RAlt), `kLMenu` (LAlt), `kLShift`, `kComma`, `kPeriod`, `kRButton` (right mouse), `mWheelUp`, `mWheelDown`, `kNumpad1`-`kNumpad9` (as spelled in `Inputs.xml`)

## Localization System

File: `JM/COT/languagecore/stringtable.csv`

CSV format — 14 languages, 16 columns (a leading `Language` key column and a trailing empty column):
```
Language,original,english,czech,german,russian,polish,hungarian,italian,spanish,french,chinese,japanese,portuguese,chinesesimp,
```
Every row must have all 16 columns; check duplicate keys and missing keys with the stringtable checks in [../systems/cleanup-playbook.md](../systems/cleanup-playbook.md) section 7.
