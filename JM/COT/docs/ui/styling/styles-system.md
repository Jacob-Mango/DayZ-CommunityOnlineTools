...
## ImageWidget: blend flags are not optional

An `ImageWidgetClass` that declares no blend properties ignores its
texture's alpha channel. A lucide icon — opaque glyph on a fully
transparent ground — then renders as an **opaque black square**, with
no error and no log line. The image loaded fine; it is being composited
wrong.

Every ImageWidget needs these:
...
`mode blend` + `"src alpha" 1` are what read the alpha channel;
`filter 1` smooths a small icon scaled off its native size. Copy the
block from `GUI/layouts/uiactions/UIActionImage.layout` rather than
writing it from memory.

> **The general rule, of which this is one half:** a widget that draws
> nothing, or draws a flat black/blank rectangle, is nearly always
> missing a *declaration* rather than being positioned wrong or
> parented wrong. A `PanelWidget` needs a `style` to have any art to
> tint (see above); an `ImageWidget` needs the blend flags to respect
> alpha. Check both before theorising about draw order, z-sort,
> clipping, or runtime widget creation.
