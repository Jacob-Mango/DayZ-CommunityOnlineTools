...
Vanilla defines a handful in `scripts/1_core/constants.c` (there is **no** `COLOR_BLACK`, `COLOR_GRAY` or `COLOR_TRANSPARENT`; write
`0xFF000000` / `ARGB( 255, 128, 128, 128 )` / `0x00000000` yourself):

```c
COLOR_WHITE   = 0xFFFFFFFF      COLOR_RED     = 0xFFF22613
COLOR_GREEN   = 0xFF2ECC71      COLOR_BLUE    = 0xFF4B77BE
COLOR_YELLOW  = 0xFFF7CA18      COLOR_CYAN    = 0xFF00FFFF
COLOR_MAGENTA = 0xFFFF00FF      // plus COLOR_*_A variants at alpha 0x1F
```

Colours for COT UI come from `JMTheme` (`Scripts/3_Game/CommunityOnlineTools/JMTheme.c`), not from these. It is the single
source of truth for the palette (daisyUI "dark" converted to ARGB ints): raw ramps (`INK_*` base ladder, primary ladder, status
set), semantic tokens (surface, border, text, accent) and a few component values, e.g. `JMTheme.WARNING`, `JMTheme.DIVIDER_MEDIUM`,
`JMTheme.SUCCESS_DIM`. Prefer a semantic token; borders and de-emphasised text are base-content at low alpha rather than a grey.
Kept in `3_Game` so every layer can read it.
