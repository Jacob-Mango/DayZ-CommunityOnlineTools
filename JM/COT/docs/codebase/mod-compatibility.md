...
DayZ-Expansion is the largest consumer: as of 2026-09-21 it has **23** `modded class` declarations over **15**
distinct COT types (`JMModuleConstructor` ×4, `JMPlayerForm` ×3, `JMPlayerModule` ×3, `JMPlayerInstance` ×2, and one each of
`JMESPMeta`, `JMESPMetaBaseBuilding`, `JMESPModule`, `JMESPViewTypeCar`, `JMESPViewTypeContainer`, `JMESPViewTypeItemBase`,
`JMESPViewTypeTent`, `JMPermissionManager`, `JMVehicleSpawnerModule`, `JMVehicleSpawnerSettings`, `JMVehiclesMenu`), plus many more files that
only call COT APIs. Re-derive the list with the loop under "Manual" below instead of trusting this count.
...
### Automated audit: not in this repo

An earlier revision of this page described `JM/COT/ai/tools/expansion_compat_check.py` (types Expansion mods exist in
COT, Expansion methods COT calls exist, every COT `modded class Expansion*` sits in its base class's layer). That
script is **not present** in this repository (there is no `ai/` folder and no history for it), so nothing here runs
it. Do the checks by hand: the loop below covers (A), and (B)/(C) are the two "before calling / before declaring" greps in
the sections above. If the script is restored, note that it needed vanilla DayZ scripts on the search path (Expansion
classes inherit `EntityAI` / `ItemBase`), that Expansion often defines what it calls (`EXP_GetMeta` lives in Expansion's
own `modded class JMESPModule`), and that only flags in `Scripts/config.cpp` `defines[]` are live (commented-out entries
such as `//"JM_COT_LOGGING"` are not).
...
1. **Check the `#ifdef` guard first.** Only blocks guarded by `JM_COT` (in
   COT's `config.cpp` `defines[]`) actually compile. `JM_COT_VEHICLEMANAGER` is
   not defined by COT (it was removed from `defines[]`), so a block Expansion
   guards with it compiles out. COT still ships empty stub types for the old
   vehicle-manager API so a third-party `modded class` over them compiles:
   `JMVehiclesMenu` (a deprecated empty `JMFormBase` in `Vehicles/JMVehiclesMenu.c`), and
   `JMVehicleSpawnerSettings` / `JMVehicleSpawnerModule` / `JMVehicleSpawnerSerialize`
   (at the top of `Vehicles/JMVehiclesForm.c`, "kept for legacy compat - do not remove").
...
Expected output (checked 2026-09-21 for the existence half): every class Expansion mods still exists in COT. The private-member
half is textual and needs the member table above as the reference.

> **Expansion-guarded COT code cannot currently be build-verified.** On the 2026-09-21 game build, booting COT with the
> `@DayZ-Expansion-Bundle` / `-Licensed` set fails inside Expansion's own AI scripts (`dayzplayerimplement.c`: "Expected
> attribute call"), unrelated to COT. `JMTerritory*`, the `DZ_Expansion_Core` blocks in `JMLoadoutModule` and
> `JMExpansionLoadoutFile` were checked by reading only. Update the game / Expansion bundle before retrying.

## Findings written: 2026-07-15 (revised 2026-09-21)